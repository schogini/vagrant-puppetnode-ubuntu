...
Vagrant.configure("2") do |config|
  ...
  config.vm.hostname = 'v-puppetnode2'
  ...
  config.vm.provision "shell", inline: <<-SHELL
    hostname v-puppetnode2
    export PUPPET_AGENT_VERSION=5.1.0 
    export UBUNTU_CODENAME=xenial 
    export DUMB_INIT_VERSION=1.2.0 
    export PUPPETSERVER_JAVA_ARGS="-Xms256m -Xmx256m" PATH=/opt/puppetlabs/server/bin:/opt/puppetlabs/puppet/bin:/opt/puppetlabs/bin:$PATH PUPPET_HEALTHCHECK_ENVIRONMENT="production"
    echo "192.168.33.10 puppet" >> /etc/hosts
    apt-get update
